...
Default_CONFIG_FILE = "MixConf.json"  # Constant for the MixConf file
#nglobal CONFIG_FILE

def load_mix_config():
    """Loads the MixConf.json file and returns a dictionary with tkinter variables."""
    if not CONFIG_FILE.get(): # If the CONFIG_FILE is empty, set it to the default value
        CONFIG_FILE.set(Default_CONFIG_FILE)
    ...
    if not check_config(raw_config):
        print("Debug: Invalid configuration detected in MixConf.json")
        return {}
    print(f"Debug: Configuration loaded successfully from {CONFIG_FILE.get()}")
...
def check_config(config):
        """Checks if the config contains the required keys."""
        required_keys = {"index", "volume", "pan", "use_for_mixdown"}
        for name, values in config.items():
            if not required_keys.issubset(values):
                print(f"Error: Missing required keys in config for track '{name}'")
                return False
        return True

def save_mix_config_as(mix_config):
    """Öffnet einen Dialog, um die MixConfig unter einem benutzerdefinierten Namen zu speichern."""
    file_path = filedialog.asksaveasfilename(defaultextension=".json", filetypes=[("JSON files", "*.json")])
    if not file_path:
        return  # Abbrechen, wenn kein Name gewählt wurde
...
def load_mix_config_from_file():
    """Öffnet einen Dialog, um eine MixConfig aus einer JSON-Datei zu laden."""
    file_path = filedialog.askopenfilename(filetypes=[("JSON files", "*.json")])
    if not file_path:
        return  # Abbrechen, wenn keine Datei gewählt wurde
    ...
    global tracks
    if not tracks:
        print("Fehler: Keine Tracks geladen!")
        return

    for track in tracks:
        name = track["name"]
        if name in raw_config:
            track["index"].set(raw_config[name].get("index", 0))
            track["volume"].set(raw_config[name].get("volume", 1.0))
            track["pan"].set(raw_config[name].get("pan", 0.5))
            track["use_for_mixdown"].set(raw_config[name].get("use_for_mixdown", True))

    print("Konfiguration geladen aus:", file_path)
...
def save_mix_config(mix_config):
    ...
    with open(CONFIG_FILE.get(), "w", encoding="utf-8") as f:
        json.dump(raw_config, f, indent=4)
...
def load_wav():
    ...
    btn_save_as.config(state=tk.NORMAL)  # Enable the "Save Mixconfig as" button
...
bottom2_frame = tk.Frame(root, bg='#00b4d8')
bottom2_frame.pack(pady=0)

# Buttons Top Frame

# Config
btn_load_config = tk.Button(top_frame, text="Load config", command=load_mix_config_from_file)
btn_load_config.pack(side=tk.LEFT, padx=5)

btn_save_as = tk.Button(top_frame, text="Save config as", command=lambda: save_mix_config_as(load_mix_config()), state=tk.DISABLED)
...
CONFIG_FILE = tk.StringVar(value="")
...
lbl_MixConf = tk.Label(bottom2_frame, textvariable=CONFIG_FILE, bg='#00b4d8', fg='white')
lbl_MixConf.pack(side=tk.LEFT, padx=5)
